...
Araclar.append(tanımlama_library.quadcopter("şimşek", [0, 0], 0, 8,    0,      0,      0,     [],    [0, 0, 0, 0]))
Araclar.append(tanımlama_library.fixedwing("dişsiz",  [0, 0], 0, 10,    0,      0,      0,     [],    2,   [0, 0]))
                                                                                                    #motor_sayısı
...
Araclar[1].duraklar.append([29.02320, 41.10182]) #ilk durak
Araclar[1].duraklar.append([29.02120, 41.10920]) #ikinci durak
...
while True:
    for arac in Araclar:
        print(f"{arac.isim} Konum: {arac.get_konum()} Hız: {round(arac.hız,2)}km/h Yön: {round(arac.yön)}° İrtifa: {arac.get_irtifa()} Motor Durumu: {arac.motor_durumu}")

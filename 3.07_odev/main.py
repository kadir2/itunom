...
Araclar = []
                                              #         Id   konum  hız   hz_ktsy yön  irtifa  drk_sycı  duraklar     motor_durumu
Araclar.append(tanımlama_library.quadcopter("şimşek", "0001",[0, 0], 0,      8,    0,      0,      0,     [],    [0, 0, 0, 0]))
Araclar.append(tanımlama_library.fixedwing("dişsiz",  "0002",[0, 0], 0,      10,    0,      0,      0,     [],    2,   [0, 0]))
                                                                                                            #motor_sayısı
...
Araclar[0].duraklar.append([29.03553, 41.11044]) #ikinci durak                                  # kordinatları x ve y değerlerini ters giriniz           
...
while True:
    for arac in Araclar:
        print(f"{arac.isim} Konum: {arac.get_konum()} Hız: {round(arac.hız,2)}km/h Yön: {round(arac.yön)}° İrtifa: {arac.get_irtifa()}m Motor Durumu: {arac.motor_durumu}   {arac.hız_katsayısı}")
    print("---------------------------------------------------------------------------------------------------")
